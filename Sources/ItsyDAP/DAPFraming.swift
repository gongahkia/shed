...
public enum DAPFramingError: Error, Equatable {
	case invalidHeader(String)
	case missingContentLength
	case invalidContentLength(String)
}

public struct DAPMessageFramer {
	...
	public static func frame(message: DAPMessage, encoder: JSONEncoder = JSONEncoder()) throws -> Data {
	...
	private static func contentLength(from headerData: Data) throws -> Int {
		guard let headerText = String(data: headerData, encoding: .ascii) else {
			throw DAPFramingError.invalidHeader("headers must be ASCII")
		}
		var length: Int?
		for line in headerText.components(separatedBy: "\r\n") where !line.isEmpty {
			let parts = line.split(separator: ":", maxSplits: 1, omittingEmptySubsequences: false)
			guard parts.count == 2 else {
				throw DAPFramingError.invalidHeader(line)
			}
			let name = parts[0].trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
			let value = parts[1].trimmingCharacters(in: .whitespacesAndNewlines)
			if name == "content-length" {
				guard let parsed = Int(value), parsed >= 0 else {
					throw DAPFramingError.invalidContentLength(value)
				}
				length = parsed
			}
		}
		guard let length else {
			throw DAPFramingError.missingContentLength
		}
		return length
	}
}
