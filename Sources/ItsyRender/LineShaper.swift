...
public struct LineShaper {
	...
	private func atlasUV(for entry: GlyphAtlasEntry, atlas: GlyphAtlas) -> AtlasUV {
		...
		return AtlasUV(
			u0: CGFloat(entry.textureX) / textureWidth,
			v0: CGFloat(entry.textureY) / textureHeight,
			u1: CGFloat(entry.textureX + entry.width) / textureWidth,
			v1: CGFloat(entry.textureY + entry.height) / textureHeight
		)
	}
}
