...
### M0.1 AX permission revocation recovery

**Goal:** Detect mid-session AX permission flips; tear down observers cleanly on revoke; re-attach on grant without dup windows or stale frames.

**Research summary (frontloaded):**
- No macOS notification exists for AX trust changes. AeroSpace polls `AXIsProcessTrustedWithOptions(nil)` ~1 Hz via `DispatchSourceTimer`. `kAXTrustedCheckOptionPrompt = true` prompts only on the first call per TCC decision; subsequent calls are silent (so polling never re-prompts).
- Fallback signal: any AX call returning `AXError.cannotComplete` or `.apiDisabled` indicates trust revocation; downgrade state immediately rather than waiting for the next poll tick.
- `AXObserver` and its `CFRunLoopSource` lingers when target pid dies — must `CFRunLoopRemoveSource` + drop ref. After revoke + re-grant, fully re-create observers (do not try to recover the old ones).

**Files to modify / add:**
- `Sources/ollyKit/AXPermission.swift:9-82` — add `static func permissionStream(interval: TimeInterval = 2) -> AsyncStream<AXPermissionStatus>` that polls on a timer and emits only on transitions.
- `Sources/ollyRuntime/OllyRuntime.swift:185-204` (`start()`) — spawn a `Task` consuming the stream.
- `Sources/ollyKit/WindowMover.swift` — add `actor` method `flushAndPause()` that drains the pending move queue (private dict around line 38) and rejects new writes until `resume()`.
- `Sources/ollyRuntime/OllyRuntimeAX.swift:93-100` — `stopAXObservers()` already exists; ensure it nils `observers` so re-attach starts clean.
- `Sources/ollyRuntime/OllyRuntimeAX.swift:120-125` — `refreshAllWindows()` is the re-attach kick. Verify `WindowStore.upsert` (`Sources/ollyKit/WindowStore.swift:167-180`) is idempotent on `WindowID` — it is.
- `Sources/ollyDSL/Hooks.swift:9-87` — add `onAXPermissionChanged(_ handler:)` HookKind (lands together with M2.1 invocation).
- `Sources/ollyIPC/IPCEventEnvelope.swift` — add `IPCEventKind.axPermission`; payload `IPCAXPermissionEvent(status: String)`.

**Implementation snippet:**
```swift
public extension AXPermission {
    static func permissionStream(interval: TimeInterval = 2) -> AsyncStream<AXPermissionStatus> {
        AsyncStream { cont in
            let q = DispatchQueue(label: "olly.ax.poll")
            let t = DispatchSource.makeTimerSource(queue: q)
            var last: AXPermissionStatus = current()
            t.schedule(deadline: .now() + interval, repeating: interval)
            t.setEventHandler {
                let now = current()
                if now != last { last = now; cont.yield(now) }
            }
...
In `OllyRuntime.start()`:
```swift
Task { [weak self] in
    for await status in AXPermission.permissionStream() {
        guard let self else { return }
        switch status {
        case .missing: await self.handleAXRevoke()
        case .trusted: await self.handleAXGrant()
        }
    }
}
```

`handleAXRevoke`: `stopAXObservers()`; `await windowMover.flushAndPause()`; `eventHub.publish(.axPermission(.missing))`; show onboarding via existing `AXPermission.presentOnboardingSheetIfNeeded`.

`handleAXGrant`: `await refreshAllWindows()`; `await startApplicationObservation()`; `for d in displays { await applyAndArrange(displayID: d.id) }`; `await windowMover.resume()`; `eventHub.publish(.axPermission(.trusted))`.

**Gotchas:**
- The system Accessibility prompt is a one-shot dialog — after a user denies, the only way to re-prompt is for them to go to System Settings. Surface the existing onboarding window controller (`Sources/ollyApp/AXOnboardingWindowController.swift:48`) automatically on revoke.
- Don't tight-loop on `.cannotComplete` — backoff 250ms × 3 before downgrading.
- AX callback closures fire on the run loop's main mode by default; ensure re-attach is invoked from main actor.

**Test plan:**
- Unit: extract `AXPermissionStatusProviding` protocol, mock returns toggled values, assert state transitions and event publication ordering.
- AX-acceptance (`OLLY_RUN_AX_ACCEPTANCE=1`): manual `tccutil reset Accessibility` and re-grant; verify no `WindowStore` duplicates and that all windows arrange on grant.

**Acceptance:**
- IPC `subscribe-events --eventKinds=axPermission` emits `{ "ok": true, "result": { "status": "missing" } }` within 2s of revoke and `"trusted"` within 2s of grant.
- After flip cycle, `ollyctl state --json | jq '.windows | length'` equals pre-revoke count.
...
- https://gannonlawlor.com/posts/macos_privacy_permissions/

---

### M0.2 Overlay panel host + in-process event bus

**Goal:** A single home for borderless `NSPanel` overlays (focus ring, drag-snap, grid, Alt-Tab, cheatsheet); a `RuntimeEventBus` actor so `ollyApp` consumes runtime events without going through UDS.

**Research summary:**
- Canonical overlay panel: `[.borderless, .nonactivatingPanel]`; `level = .screenSaver` (Alt-Tab/grid full-screen) or `.statusBar` (above menubar) or `.floating` (focus ring); `collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary, .stationary, .ignoresCycle]`; `isOpaque = false`; `backgroundColor = .clear`; `hasShadow = false`; `ignoresMouseEvents` toggleable; `canBecomeKey/Main = false`.
- Reconfigure on `NSApplication.didChangeScreenParametersNotification`; rebuild per-`NSScreen` panel arrays.
- JankyBorders rebuilds borders on every `kAXWindowMoved`/`kAXWindowResized` notification with explicit `setFrame` (no animation, no lag); uses `CAShapeLayer` stroke per window.

**Files to add:**
- `Sources/ollyApp/Overlays/OverlayPanel.swift` — `final class OverlayPanel: NSPanel` with the constructor above.
- `Sources/ollyApp/Overlays/OverlayPanelHost.swift` — `@MainActor final class OverlayPanelHost`: holds `[NSScreen.localizedName: OverlayPanel]`, `rebuildOverlays()` triggered by screen-param change, fade helpers extracted from `Sources/ollyApp/OverviewModeController.swift:82`.
- `Sources/ollyApp/Overlays/OverlayRegistry.swift` — singleton mediating overlay collisions (e.g. command palette hides snap preview).
- `Sources/ollyRuntime/RuntimeEventBus.swift` — `actor RuntimeEventBus` exposing `AsyncStream<IPCEvent>` for in-process consumers. `OllyRuntime` publishes to both UDS subscribers and bus.
...
final class OverlayPanel: NSPanel {
    init(screen: NSScreen, level: NSWindow.Level = .floating, clickThrough: Bool = true) {
        ...
        ignoresMouseEvents = clickThrough; isFloatingPanel = true
        hidesOnDeactivate = false; worksWhenModal = true
    }
...
public actor RuntimeEventBus {
    ...
    public func subscribe() -> AsyncStream<IPCEvent> {
        AsyncStream { cont in
            let id = UUID(); continuations[id] = cont
            cont.onTermination = { [weak self] _ in Task { await self?.remove(id) } }
        }
    }
    public func publish(_ ev: IPCEvent) { continuations.values.forEach { $0.yield(ev) } }
    private func remove(_ id: UUID) { continuations.removeValue(forKey: id) }
}
```

**Gotchas:**
- A `.screenSaver` overlay sits above the menu bar but below Apple's Mission Control overlay — fine for our purposes.
- `NSScreen.localizedName` is not unique across identical models — use `CGDirectDisplayID` (NSScreen's `deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")]`) as the key.
- The `AsyncStream` from `RuntimeEventBus` is consumed on `@MainActor` — wrap UI updates accordingly.

**Test plan:**
- UI test: open + close overlay 100×; assert `OverlayPanelHost.activeCount == 0` afterwards (SoakHarness scenario).
- Unit: `RuntimeEventBus` publishes to multiple subscribers; cancelled subscribers are removed.

**Acceptance:**
- All five M3 overlays inherit from `OverlayPanel` and register with `OverlayPanelHost`.
- `ollyApp` consumes runtime events via `RuntimeEventBus.subscribe()` from at least one controller.
...
### M0.3 AX drag-session derivation

**Goal:** Public-API derivation of drag-start / drag-end / drag-frame events. Used by M3.2 drag-snap preview.

**Research summary:**
- No native AX drag-start notification. Standard heuristic: first `kAXWindowMovedNotification` per window = drag-start (debounce 16ms); >120ms without further `kAXWindowMoved` or arrival of `kAXFocusedWindowChangedNotification` / `kAXMainWindowChangedNotification` = drag-end.
- Filter olly-initiated moves: compare against `WindowMover.lastFrames` (private dict at `Sources/ollyKit/WindowMover.swift:38`) — if the notification frame matches what we just wrote, drop it.
- Mouse position via `CGEvent(source: nil)?.location` to test against snap zones during drag.

**Files to add:**
- `Sources/ollyKit/AXDragSession.swift` — `actor AXDragSession` consuming `AXObserver` move events, emitting `.started(windowID, frame, mouse)`, `.moved(windowID, frame, mouse)`, `.ended(windowID, finalFrame)`.

**Implementation snippet:**
```swift
public enum AXDragEvent {
    case started(WindowID, CGRect, NSPoint)
    case moved(WindowID, CGRect, NSPoint)
    case ended(WindowID, CGRect)
}
public actor AXDragSession {
    private var activeWindow: WindowID?
    private var lastTick: ContinuousClock.Instant = .now
    private var endTask: Task<Void, Never>?
    private var stream: AsyncStream<AXDragEvent>.Continuation?

    public func feed(windowID: WindowID, frame: CGRect, ourLastFrame: CGRect?) {
        if let our = ourLastFrame, abs(our.minX - frame.minX) < 1, abs(our.minY - frame.minY) < 1 { return }
        let mouse = NSEvent.mouseLocation
        if activeWindow == windowID {
            stream?.yield(.moved(windowID, frame, mouse))
        } else {
            activeWindow = windowID
            stream?.yield(.started(windowID, frame, mouse))
        }
        lastTick = .now
        endTask?.cancel()
        endTask = Task { [weak self] in
            try? await Task.sleep(nanoseconds: 120_000_000)
            await self?.endIfIdle(windowID: windowID, frame: frame)
        }
    }
    private func endIfIdle(windowID: WindowID, frame: CGRect) {
        guard activeWindow == windowID else { return }
        stream?.yield(.ended(windowID, frame))
        activeWindow = nil
    }
}
```

Wire into `Sources/ollyRuntime/OllyRuntimeAX.swift:handle(axEvent:)` (~line 134) — call `await dragSession.feed(windowID:frame:ourLastFrame:)` on `windowMoved`.

**Gotchas:**
- Window-resize is NOT a drag event; key off `kAXWindowMovedNotification` only.
- Programmatic moves from rules-engine still trigger AX move events — the `WindowMover.lastFrames` cross-check is mandatory.
- 120ms is empirical; observe drag at 60Hz means events every ~16ms during active drag, so 120ms is ~7 frames of inactivity.

**Test plan:**
- Unit: feed synthetic move bursts with controlled timestamps; assert state machine transitions.
- AX-acceptance: drag a real window via UI; assert one `.started` + N `.moved` + one `.ended`.

**Acceptance:**
- `AXDragSession.subscribe()` returns an `AsyncStream<AXDragEvent>` with the three event cases.
- Soak test under repeated drag: no events leak after window destroyed.
