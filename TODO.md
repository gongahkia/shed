...
(B) 2026-06-26 Add `FocusStack.swift`: maintain MRU per (display, tag-set); restore focus on tag switch +phase1 @swift
...
(A) 2026-06-26 Define `LayoutEngine` protocol: `id`, `displayName`, associated `Config` type, `arrange(windows: [WindowSnapshot], in bounds: CGRect, focus: WindowID?) -> [Placement]` +phase3 @swift @layout ref:N§4-D4
(A) 2026-06-26 Define `Placement` struct: `windowID`, `frame: CGRect`, `zOrder: Int`, `hidden: Bool`; deterministic given identical inputs +phase3 @swift @layout
...
(B) 2026-06-26 Snapshot tests: feed a fixed `[WindowSnapshot]` to each engine, golden-file the `[Placement]` output, diff on PRs +phase4 @swift @layout
...
(A) 2026-06-26 Implement menubar app target with `NSStatusItem` icon; menu shows active display, active tags, current engine, quick-actions +phase7 @swift @ui
...
(A) 2026-06-26 Add `extensions/sketchybar/` with example `sketchybarrc` consuming `ollyctl events`; document install in README +phase8 @ecosystem @docs ref:N§7
