...
(A) +Phase18-SymbolNav @nav id:601 est:30m Add #-prefix mode for current-file symbols. When palette query starts with `#`, scope to the active document only via `WorkspaceIndex.symbolsForFile(url)`. Same row format, no path column.
(A) +Phase18-SymbolNav @nav id:602 est:1h dep:600,601 Bind Cmd-T → "Go to symbol in workspace" (opens palette pre-filled with `@`). Bind Cmd-Shift-O → "Go to symbol in file" (pre-filled with `#`). Add corresponding items under Navigate menu. Update `docs/keymap-reference.md` via existing generator.
...
(B) +Phase18-SymbolNav @ui id:606 est:1h Add "Indexing N/M" indeterminate→determinate progress in status bar during initial `WorkspaceIndex` scan; hide when complete. Reuse `NSProgressIndicator(style: .bar)` sized to status-bar height.
...
(A) +Phase19-LSP @lsp id:620 est:3h Server registry: `~/.config/itsy/lsp.toml` maps `language-id` → `{ command, args, root-patterns, init-options, settings }`. Bundle defaults: `swift` → `xcrun --find sourcekit-lsp` (no args), `typescript`/`javascript` → `typescript-language-server --stdio`, `rust` → `rust-analyzer`, `python` → `pyright-langserver --stdio`, `go` → `gopls`. Root-pattern matching: walk up from document until any of `[Package.swift, package.json, Cargo.toml, pyproject.toml, go.mod, .git]` is found per server. ref:https://github.com/swiftlang/sourcekit-lsp
(A) +Phase19-LSP @lsp id:621 est:2h dep:620 LSP manager (`Sources/ItsyEditor/LSPManager.swift`): lazy-spawn one server per `(language-id, workspace-root)` pair; reuse existing `ItsyLSP` `ProcessClient`/`Session`. Lifecycle: spawn on first matching `didOpen` → `initialize` → `initialized` → forward `didOpen` → on workspace close `shutdown` → `exit`. Bounded retry on crash: 3 spawns within 60 s window, then mark server `failed` and surface in status bar.
(A) +Phase19-LSP @lsp id:622 est:2h dep:621 Document sync: `ItsyDocument.didOpen` → `textDocument/didOpen`; on edit, debounce 100 ms then send `textDocument/didChange` (full-sync if `server.capabilities.textDocumentSync.change == 1`, incremental if `== 2`); on close → `textDocument/didClose`. Strictly monotonic `version` field. Assert via test that out-of-order edits never reach the server. ref:https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocument_synchronization
(A) +Phase19-LSP @lsp id:623 est:2h dep:622 Diagnostics bridge: handle `textDocument/publishDiagnostics` notification → push to `WorkspaceProblems`. Map `DiagnosticSeverity` 1-4 → Error/Warning/Information/Hint with distinct icons + colors. Preserve `relatedInformation` for tooltip. Group by URI; replace-all on each notification. ref:https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocument_publishDiagnostics
(A) +Phase19-LSP @ui id:624 est:3h dep:623 Gutter diagnostics: render severity markers in left gutter at each diagnostic's start line. Hover marker → `NSPopover` showing `diagnostic.message` + `relatedInformation` entries as clickable file:line links. Click marker → focus Problems panel and select the matching row. Use existing renderer's gutter region; add a `GutterDecorator` protocol if not present. ref:https://developer.apple.com/documentation/appkit/nspopover
(A) +Phase19-LSP @lsp id:625 est:2h dep:621 Capabilities advertisement in `initialize` params — under `capabilities.textDocument`: `documentSymbol.hierarchicalDocumentSymbolSupport=true`, `completion.completionItem.{snippetSupport=true, labelDetailsSupport=true, resolveSupport.properties=["documentation","detail","additionalTextEdits"]}`, `publishDiagnostics.relatedInformation=true`, `hover.contentFormat=["markdown","plaintext"]`, `definition.linkSupport=true`, `rename.prepareSupport=true`. Under `capabilities.workspace`: `applyEdit=true`, `workspaceEdit.documentChanges=true`, `configuration=true`.
(A) +Phase19-LSP @ui id:626 est:5h dep:622,625 Completion popup: on `Ctrl-Space` or server-declared trigger character, send `textDocument/completion`. Caret rect: compute via `ItsyRender` line metrics + rope offset → screen point. Render results in `NSPanel` (`.nonactivatingPanel`, `.fullSizeContentView`) positioned below caret (flip above if clipped). Filter locally on subsequent keystrokes when `isIncomplete=false`; otherwise re-request. Tab/Enter accept (apply `textEdit` or `insertText`); Esc dismiss; Up/Down move selection. Honor `InsertTextFormat=Snippet` with `$1`, `${2:foo}` tab stops driven by existing multi-cursor support (id:240). ref:https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocument_completion
(B) +Phase19-LSP @lsp id:627 est:2h dep:626 `completionItem/resolve`: on popup selection-change, debounce 200 ms then request resolve for the selected item. Preserve `data` field across resolve (per LSP spec). Render resolved `documentation` (MarkupContent) in a detail popover beside the completion list. Cache resolved items by id for the duration of the popup session. ref:https://docs.rs/lsp-types/latest/lsp_types/struct.CompletionItem.html
(A) +Phase19-LSP @ui id:628 est:3h dep:622,625 Hover tooltip: on Cmd-hover sustained 500 ms over an identifier (or keybind `K` in vim profile), send `textDocument/hover`. Render `MarkupContent` (markdown) via a minimal markdown→`NSAttributedString` converter (headings, code spans, code blocks, paragraphs, links — no tables/images). Show in `NSPopover` anchored to caret rect via `positioningRect`.
(A) +Phase19-LSP @lsp id:629 est:2h dep:622 Go-to-definition: on Cmd-Click on identifier (or F12 / `gd` in vim profile) send `textDocument/definition`. Handle `Location | Location[] | LocationLink[]`. If single → jump immediately; if multiple → show picker reusing references-panel UI. Push current cursor onto a jump-history stack; Cmd-[ / Cmd-] navigate the stack.
(B) +Phase19-LSP @ui id:630 est:4h dep:622 References panel: command "Find all references" (Shift-F12 / `gr` in vim profile) sends `textDocument/references` with `context.includeDeclaration=true`. Show results in a new `WorkspaceProblems`-style panel grouped by file with 1-line context preview. Double-click jumps. Persist last-results until next request.
(B) +Phase19-LSP @lsp id:631 est:4h dep:622 Rename flow: F2 (or `<leader>rn` in vim) sends `textDocument/prepareRename` to validate cursor target. If accepted, show inline floating input field at current symbol range pre-filled with current name; on submit send `textDocument/rename` with new name. Apply returned `WorkspaceEdit` via a single `workspace/applyEdit` handler that opens/edits all affected documents atomically inside one undo group. Roll back all changes if any edit fails. ref:https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocument_rename
(B) +Phase19-LSP @lsp id:632 est:3h dep:622 Code actions: on Opt-Enter near a diagnostic (or `<leader>ca`) send `textDocument/codeAction` with `context.diagnostics` populated from the current diagnostics-for-range. Show titles in `NSMenu` at caret; on selection, execute the command (`workspace/executeCommand`) or apply the inline edit. Filter to `CodeActionKind.QuickFix` initially; later expose Refactor / Source actions behind a Cmd-. shortcut. ref:https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocument_codeAction
(B) +Phase19-LSP @lsp id:633 est:2h dep:622 Formatting: Cmd-Opt-F invokes `textDocument/formatting`; with selection invokes `textDocument/rangeFormatting`. Apply returned `TextEdit[]` in reverse-offset order (preserve earlier offsets) inside a single undo group. Format-on-save toggle in settings (default off).
(B) +Phase19-LSP @lsp id:634 est:2h dep:621 Signature help: on server-declared trigger characters (typically `(` and `,`), send `textDocument/signatureHelp`. Show in `NSPopover` above caret with active parameter rendered bold. Dismiss on `)` or Esc.
(C) +Phase19-LSP @lsp id:635 est:2h dep:621 `workspace/configuration` handler: respond with values from `~/.config/itsy/lsp.toml`'s per-server `[settings]` table (e.g., rust-analyzer's `checkOnSave`, pyright's `reportMissingImports`). Section names follow LSP request's `section` field.
(C) +Phase19-LSP @perf id:636 est:2h dep:622 LSP cold-start budget verification: instrument `bench/scripts/regression.sh` to assert that opening a Swift file does not regress Itsy cold-start KPI (<150 ms) by spawning sourcekit-lsp during launch. Spawn must be strictly lazy on first matching `didOpen`. Add a separate KPI: "Time from didOpen to first publishDiagnostics" with target <5 s for sourcekit-lsp on `large.ts` equivalent.
...
(A) +XCut @docs id:410 est:1h Rewrite `NORTHSTAR.md` to reflect actual scope. Move `LSP / code completion / hover / diagnostics`, `Debugger / DAP`, `Extension marketplace, plugin runtime`, `Git UI (use the terminal)` from `Scope — OUT` to a new `Scope — IN (post-v0.1)` section. Keep in `Scope — OUT`: `AI assistance`, `Integrated terminal`, `Collaboration / multiplayer`, `Linux/Windows ports`, `iOS/iPadOS`, `Telemetry`. Update principle #5: replace "No AI, no collab, no terminal, no marketplace, no telemetry. Forever." with "No in-process AI runtime, no integrated terminal, no collaboration, no telemetry. Agent hosting via ACP protocol is allowed as a process-boundary integration." (decide separately whether to commit to ACP). Update principle #6 to "Language servers and debug adapters via process boundary only. No in-process plugin host through v1.0." Update principle #7 LOC target from <15 kLOC to a realistic figure once `tokei .` is run on current state. Add new KPI rows: "RSS delta per active LSP server" (<150 MB sourcekit-lsp on Swift workspace), "Time from didOpen to first publishDiagnostics" (<5 s). Re-affirm cold-start KPI <150 ms still gates v0.1; current 272 ms (per `bench/notes/coldstart-audit.md`) remains the release blocker independent of any feature work below.
