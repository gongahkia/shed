...
@Test func bundledPlainProfileDefinesMacStandardAppBindings() throws {
	...
	#expect(engine.handle(try keyEvent("p", modifiers: [.command])) == .command("nav.gotoFile"))
	...
	#expect(engine.handle(try keyEvent("g", modifiers: [.control])) == .command("nav.gotoLine"))
...
@Test func bundledVimProfileDefinesReplaceCaseIndentAndFormat() throws {
	...
	#expect(engine.handle(try keyEvent("f")) == .command("nav.gotoFile"))
	#expect(engine.handle(try keyEvent(" ", keyCode: 49)) == .partial)
...
@Test func bundledEmacsProfileDefinesStandardMotions() throws {
	...
	#expect(engine.handle(try keyEvent("x", modifiers: [.control])) == .partial)
	#expect(engine.handle(try keyEvent("f", modifiers: [.control])) == .command("nav.gotoFile"))
	#expect(engine.handle(try keyEvent("x", modifiers: [.control])) == .partial)
