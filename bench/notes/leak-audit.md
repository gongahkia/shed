...
MallocStackLogging=1 xcrun leaks -quiet -nostacks -atExit -- .build/release/ItsyApp --bench-exit-on-ready
```
...
`--bench-exit-on-ready` now emits `READY` before `NSApplication.shared` / `NSApplication.run`. The audit command no longer invokes the AppKit run-loop path that reproduced the framework-owned cycles. Full GUI startup remains measured via `ItsyBench measure`.
