...
`WorkspaceController` owns the ordered folder set and the Explorer's selected folder. `WorkspaceManifest` is its portable import/export boundary; it accepts only validated local folder entries and ignores all settings and extension data. An explicitly imported standard `.code-workspace` may additionally remain a read-only source for the existing strict task and launch compatibility importers while its resolved folder set still exactly matches. Resource-scoped controllers resolve the deepest configured folder containing the current file before falling back to that selection. This keeps sibling projects isolated for task discovery, extension SCM, Dev Container actions, and extension workspace tools without changing the Explorer merely by switching files.
