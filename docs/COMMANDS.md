...
| `:test watch`, `:test continuous` | Start an explicit session-only local recursive watcher, run tests once, then coalesce source changes into follow-up Run All jobs; requires a prior refresh and is unavailable for connected remote or Dev Container roots |
| `:test unwatch`, `:test watch off` | Stop continuous local test watching and discard a queued follow-up run |
...
| `:debug import [vscode]`, `:debug imports`, `:debug forget-imports` | Explicitly snapshot the currently accepted VS Code launch subset into private Shed session state, inspect it, or remove it; snapshots are named `imported:vscode:<name>`, never write project files, and still require an explicitly configured adapter and start |
| `:debug select <name>`, `:debug start [name] [-- input=value ...]`, `:debug stop`, `:debug restart [name] [-- input=value ...]` | Control an explicit DAP session, including user-installed Python/Go profiles plus explicit compiled-artifact C#/.NET and native LLDB adapters; declared native launch inputs can override defaults after `--`, while a configured pre-launch workspace task or a label mapped from an accepted VS Code process or POSIX-shell task must succeed before its adapter starts |
...
| `:debug function list` | Open persisted function breakpoints for the selected workspace |
| `:debug function add\|enable\|disable\|remove <name>` | Add or update a named function breakpoint; it is synchronized only with a declared and runtime-advertised DAP function-breakpoint adapter |
| `:debug function condition\|hit <name> -- <value>` | Set a bounded function-breakpoint condition or hit condition; `--` separates a name that may contain spaces from its value |
...
| `:debug set <reference> <name> -- <value>` | Change a displayed paused variable only when the adapter was declared with `set_variable` and advertises `supportsSetVariable` |
...
| `:lsp type definition`, `:lsp type-definition`, `:typedefinition`, `:typedef` | LSP go-to-type-definition |
...
| `:lsp selection`, `:lsp selection expand` | Expand the active selection to the next larger LSP selection range at the caret |
