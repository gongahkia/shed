...
| `:debug import [vscode]`, `:debug imports`, `:debug forget-imports` | In a trusted workspace, explicitly snapshot the currently accepted VS Code launch subset into private Shed session state, inspect it, or remove it; snapshots are named `imported:vscode:<name>`, never write project files, and still require an explicitly configured adapter and start |
