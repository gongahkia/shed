...
| `terminal.default.profile` | `system` | string | Default terminal: `system` or a detected `builtin:<id>` profile |
...
Open **Language Services** in Settings or run `:lsp manage` for local detection and managed installs. Nothing downloads, installs, updates, or changes LSP configuration on startup, file open, detection, or a command alone: every install/update requires a fresh **Yes** in the panel's review dialog. Java uses the SHA-256-pinned Eclipse JDT LS archive, which Shed verifies before extraction under `~/.shed/managed-languages/`. Pyright, TypeScript/JavaScript, JSON, HTML/CSS, and Markdown use exact npm package versions in the same cache, never a global npm installation; their review discloses that dependency integrity is recorded in the local npm lockfile rather than verified against an independently published archive checksum. npm lifecycle scripts, audit, funding, and update notifications are disabled. Shed writes the managed launcher and required arguments for every file type covered by the selected service, then restarts those LSP clients. gopls, rust-analyzer, and clangd remain user-managed because their official paths depend on the user's Go/Rust/LLVM toolchains. Full ownership and cache policy: [Managed Language Support Trust Model](MANAGED_LANGUAGE_SUPPORT.md).

User-managed `lsp.<ext>.command` and `lsp.<ext>.args` launch direct arguments rather than an implicit shell. Arguments are separated by whitespace; matching single or double quotes retain a value containing spaces, and a malformed quoted value appears as an LSP startup error. This permits an explicit PowerShell Editor Services (PSES) stdio setup after the user installs PowerShell 7 and extracts the upstream PSES bundle. Replace both paths below with that bundle's absolute path, then run `:lsp restart ps1`:

```toml
"lsp.ps1.command" = "pwsh"
"lsp.ps1.args" = "-NoLogo -NoProfile -Command \"& '/absolute/path/PowerShellEditorServices/Start-EditorServices.ps1' -BundledModulesPath '/absolute/path' -HostName 'Shed' -HostProfileId 'shed' -HostVersion '2.0.0' -Stdio\""
```

Shed sends `.ps1`, `.psm1`, and `.psd1` documents to that configured server with the `powershell` language id. This is LSP-only: PSES's named-pipe Debug Adapter Protocol and terminal transports are not configured by Shed.

Shed keeps an independent client for each `(file extension, workspace root)` pair, so files from separate projects do not replace each other's server. A server that advertises incremental document synchronization receives debounced range changes; others receive a full-document update for compatibility. Semantic tokens and inlay hints render automatically when supported, with inline rendering controlled separately below. Document symbols use an on-demand LSP request with local fallback. `:lsp codelens` explicitly lists capability-gated CodeLens commands, resolves lenses when the server supports resolution, and executes only a separately selected lens through the server's advertised command endpoint. Repeated `:lsp selection` commands ask a compatible server for the caret's nested selection ranges and apply the next larger valid range in the active buffer. `:lsp links` lists resolved document links; only an explicit index can open a local file or `http`, `https`, or `mailto` target. `:lsp colors` lists server-reported document ranges as normalized RGBA hex colors without changing the buffer. `:lsp pulldiagnostics` explicitly refreshes an advertised pull-diagnostic provider; a full response replaces the current URI's diagnostic snapshot, while an unchanged response retains it. `:lsp workspacediagnostics` asks each active server that advertises workspace diagnostics for one bounded direct report and opens returned local-file entries in quickfix; it does not request optional progress-streamed partial reports. `:lsp folding` lists server-reported folding ranges without changing the document; the existing interactive fold model remains Markdown-heading-only. `:workspace symbols <query>` first queries usable active LSP servers; if none produces a result, it performs one explicit asynchronous local scan across workspace folders. Java files use the JDK parser only for recovered declaration names; every other language remains lexical. The scan respects the workspace ignore filter, reads only regular UTF-8 files at most 2 MiB each, returns at most 300 matches, and either revalidates the existing opt-in workspace file index or scans without persisting it. It does not build a background symbol index, resolve Java types, compile source, or replace LSP symbols when an LSP result exists.

`remote.lsp.enabled` is a global opt-in for a connected SSH, Docker-container, or WSL workspace, or for an already running local Dev Container. It requires an explicit global `lsp.<ext>.command` for a server already installed in that remote environment. Shed starts that one direct-argv server through the provider and translates only files under the local mirror to its remote `file:` URIs; for a Dev Container it first asks the user-installed CLI for the mounted workspace path. Run `:lsp restart <ext>` after changing this setting; a live client retains the URI mode it started with. It does not install a remote server, run managed local artifacts remotely, bootstrap a host, or make test or arbitrary host processes remote. An SSH login banner on stdout corrupts the LSP stream and must be disabled in the user's SSH setup.
...
| `lsp.code.lens.enabled` | `true` | bool | CodeLens requests and resolution |
| `lsp.selection.ranges.enabled` | `true` | bool | Selection-range requests |
| `lsp.document.links.enabled` | `true` | bool | Document-link requests and resolution |
| `lsp.document.colors.enabled` | `true` | bool | Document-color requests |
| `lsp.pull.diagnostics.enabled` | `true` | bool | Explicit pull-diagnostic requests |
| `lsp.workspace.diagnostics.enabled` | `true` | bool | Explicit direct workspace-diagnostic requests |
| `lsp.folding.ranges.enabled` | `true` | bool | Explicit folding-range requests |
...
| `lsp.format.on.save.enabled` | `false` | bool | Default format-on-save policy for file types without an override; an unavailable, failed, or stale formatting request leaves the buffer unsaved |
...
Each file type defaults to `lsp`. Use `:formatter` for the active file type or set the dynamic keys below. `external` launches the configured command with direct argv only, sends the buffer on stdin, reads formatted stdout, uses the workspace as its working directory, and never invokes a shell. `${file}` is permitted as one complete argument. `disabled` skips formatting for that extension. `formatter.<ext>.format.on.save` overrides `lsp.format.on.save.enabled` for that file type; formatting runs asynchronously and an error or stale buffer leaves the file unwritten.
...
See [Network and telemetry boundary](NETWORK_PRIVACY.md) for the complete app-owned outbound-path audit, including Git remotes, loopback debug adapters, browser delegation, and user-controlled child processes.
...
| `lsp.<ext>.command` | LSP server command for file type | `"lsp.py.command" = "pyright-langserver"` |
| `lsp.<ext>.args` | Quoted direct argv text for an LSP server | `"lsp.py.args" = "--stdio"` |
...
| `formatter.<ext>.format.on.save` | Per-file-type format-on-save override | `"formatter.py.format.on.save" = true` |
...
| `debug.configuration.<name>.<field>` | DAP launch/attach configuration; a launch chooses exactly one `program`, `module`, or `code` target (`file_extensions` applies only to `program`; `prelaunch_task` normally names a workspace `.shedtasks` task) | `"debug.configuration.main.request" = "launch"` |
