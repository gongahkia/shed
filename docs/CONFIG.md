...
Shed sends `.ps1`, `.psm1`, and `.psd1` documents to that configured server with the `powershell` language id. This is LSP-only: PSES's named-pipe Debug Adapter Protocol and extension terminal transports are not configured by Shed.
