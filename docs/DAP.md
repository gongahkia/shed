...
`ReferenceDebugAdapter` is an in-process framed DAP fixture used by automated tests. `ReferenceTcpDebugAdapter` is a child JVM fixture for the spawned-loopback transport. Together they exercise initialize, delayed launch completion, `initialized`, full-source and function breakpoints, `configurationDone`, stopped-frame inspection, watch evaluation, disconnect, timeout cancellation, malformed adapter output, spawned TCP readiness, and process cleanup without relying on a platform-specific debugger executable.
