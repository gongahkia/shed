...
Adapter identifiers and configuration names are `[A-Za-z0-9_-]+`. `transport` is `stdio` (default) or `tcp`; `stdio` requires `command`, while `tcp` must not set one. Capabilities are comma-separated: `launch`, `attach`, `configuration_done`, `breakpoints`, `function_breakpoints`, `data_breakpoints`, `exception_breakpoints`, `conditional_breakpoints`, `hit_conditional_breakpoints`, `log_points`, `threads`, `stack_trace`, `scopes`, `variables`, `set_variable`, `evaluate`, `continue`, `next`, `step_in`, `step_out`, `pause`, `goto`, `reverse_continue`, `step_back`, `restart_frame`, `exception_details`, `modules`, and `loaded_sources`.
