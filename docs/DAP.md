...
- Shed includes two narrow local profiles: `python-debugpy` for the separately user-installed `debugpy-adapter` executable, and `go-delve` for the separately user-installed `dlv` executable. Shed neither bundles, downloads, nor probes either debugger; each profile is unavailable until its executable is on `PATH`.
