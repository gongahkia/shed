...
Shed never starts a debug adapter while inspecting configurations or selecting one. A rejected configuration, adapter start error, timeout, or failed DAP response leaves the session `FAILED` with diagnostics visible in `[debug status]`; normal editing remains available. Generic launch arguments are exactly one of `program`, `module`, or `code`, plus `cwd`, `args`, an imported bounded `env` map, and validated literal adapter options. Attach arguments are `host`, `port`, `cwd`, `args`, and the same option boundary. Test debugging never guesses a framework or adapter; it only starts the explicit mapping in `.shedtests`.
