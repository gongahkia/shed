...
Large-file mode is a read-only file-buffer state with bounded source windows. It does not retain hidden full content; its unchanged source can be streamed through atomic save. Shed reports when the automatic large-file path is active or unavailable.
