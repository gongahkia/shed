...
| Debugging | DAP client with persisted source breakpoints (enabled/condition/hit/log when adapter-declared), explicit Continue/Next/Step In/Out/Pause when adapter-declared, stack/variables/watches; one Python-only profile for a separately installed `debugpy-adapter`; manual configuration and extensions for everything else | Zed supplies adapters for several languages and contextual debug tasks; VS Code has a broad debugger-extension model. [Zed debugger](https://zed.dev/docs/debugger) | A narrow local Python path; not turnkey multi-language debugging |
