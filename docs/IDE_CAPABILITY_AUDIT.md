...
| Languages | Ten managed language-service catalog entries; built-in lexical support; direct-argv LSP; VS Code-compatible user snippets; optional LSP semantic-token rendering; extension language profiles for extensions/file names/literal first lines, keywords, comments, strings, indentation preferences, and explicit buffer-local lexical selection | VS Code supports declarative grammars, injections, snippets and rich language APIs. Zed uses Tree-sitter grammar/query assets for highlighting, structure, indentation, injections, runnable-code detection, and more. [VS Code capabilities](https://code.visualstudio.com/api/extension-capabilities/overview), [Zed languages](https://zed.dev/docs/extensions/languages) | Useful, but partial language platform |
| Debugging | DAP client with persisted source breakpoints (enabled/condition/hit/log only when both configured and advertised at DAP initialization), adapter-filter exception-breakpoint enable/disable, explicit Continue/Next/Step In/Out/Pause when adapter-declared, stack/variables/watches; one Python-only profile for a separately installed `debugpy-adapter`; manual configuration and extensions for everything else | Zed supplies adapters for several languages and contextual debug tasks; VS Code has a broad debugger-extension model. [Zed debugger](https://zed.dev/docs/debugger) | A narrow local Python path; not turnkey multi-language debugging |
...
It still does **not** add TextMate grammars, Tree-sitter parsing, extension-defined grammar/query injections, language-specific folding, semantic-token production, formatter registration, or extension-defined snippets. Built-in lexical highlighting does inject JavaScript/CSS in HTML and known languages in Markdown fences, but that is not a structural grammar platform. `:language` can select a lexical profile for the current buffer, but it does not retarget an LSP server. LSP behavior is limited to what the server and Shed's existing LSP client expose.

Outside profiles, Shed loads user `.json` and `.code-snippets` files using a bounded VS Code-compatible snippet subset and can render semantic tokens only when an already configured language server provides them. Those are useful editor features, but they do not create a structural grammar or a broad language-extension ecosystem.

### Debugging

Shed now has a real local Python launch path when the user has installed upstream `debugpy-adapter`: an explicit start can choose its profile for the active `.py` or `.pyw` file when nothing is selected; it runs an explicitly configured local pre-launch workspace task before opening the adapter, then performs DAP initialization, persisted source-breakpoint synchronization (including enabled, conditional, hit-count, and log-point fields), adapter-filter exception-breakpoint enable/disable, `configurationDone` when supported, stopped-frame inspection, watch evaluation, and explicit Continue/Next/Step In/Out/Pause controls. Other debugger adapters remain user-configured or extension-contributed; rich source breakpoint fields require both local configuration and the matching capability advertised at DAP initialization, while execution controls require a local declaration.

It still does **not** discover debug configurations from the current language or project beyond the narrow explicit Python fallback, provide module/code launch targets, manage Python environments, provide turnkey test-framework debugger setups, offer adapter-specific exception conditions/options beyond per-filter enable/disable, navigate automatically to a stopped source location, support reverse execution or run-to-cursor, or operate a remote debug adapter. `.shedtests` can explicitly map a discovered test selection to a user-provided DAP configuration, but Shed does not infer that mapping or debugger arguments. Zed documents automatic scenarios for Rust, Go, Python, JavaScript, and TypeScript, pre-debug build tasks, and richer exception-breakpoint management; Shed should not claim comparable debugger coverage. [Zed debugger](https://zed.dev/docs/debugger)
...
This does not relocate LSP, test discovery, debugging, extension code, or file watching by default. A globally enabled, explicitly user-configured `lsp.<ext>.command` may run through an SSH, container, or WSL workspace and receives translated remote-root `file:` URIs; it has no remote installer, host bootstrap, persistence/reconnect mechanism, or remote extension host. `:remote terminal` is an explicitly selected PTY bridge; normal terminals remain local. A task can be explicitly targeted with `:task remote`; it carries only its direct argv, workspace-relative cwd, and declared environment. SSH remains a remote shell boundary; command execution has the permissions of the selected remote account or container. This remains materially different from VS Code and Zed remote servers, which host core project tooling remotely.
