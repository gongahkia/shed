...
Shed now has a real local Python launch path when the user has installed upstream `debugpy-adapter`: it performs DAP initialization, persisted source-breakpoint synchronization (including enabled, conditional, hit-count, and log-point fields), `configurationDone` when supported, stopped-frame inspection, watch evaluation, and explicit Continue/Next/Step In/Out/Pause controls. Other debugger adapters remain user-configured or extension-contributed; rich source breakpoint fields require both local configuration and the matching capability advertised at DAP initialization, while execution controls require a local declaration.
