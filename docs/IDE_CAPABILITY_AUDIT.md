...
| Debugging | DAP client with source breakpoints, explicit Continue/Next/Step In/Out/Pause when adapter-declared, stack/variables/watches; one Python-only profile for a separately installed `debugpy-adapter`; manual configuration and extensions for everything else | Zed supplies adapters for several languages and contextual debug tasks; VS Code has a broad debugger-extension model. [Zed debugger](https://zed.dev/docs/debugger) | A narrow local Python path; not turnkey multi-language debugging |
...
Shed now has a real local Python launch path when the user has installed upstream `debugpy-adapter`: it performs DAP initialization, source-breakpoint synchronization, `configurationDone` when supported, stopped-frame inspection, watch evaluation, and explicit Continue/Next/Step In/Out/Pause controls. Other debugger adapters remain user-configured or extension-contributed; a capability declaration is required before Shed sends an execution control.

It still does **not** discover debug configurations from the current language or project, run a build task before launch, provide module/code launch targets, manage Python environments, provide test-framework debugging, offer conditional/log/hit-count/disabled/exception breakpoints, navigate automatically to a stopped source location, support reverse execution or run-to-cursor, or operate a remote debug adapter. Zed documents automatic scenarios for Rust, Go, Python, JavaScript, and TypeScript, pre-debug build tasks, and those richer breakpoint controls; Shed should not claim comparable debugger coverage. [Zed debugger](https://zed.dev/docs/debugger)
