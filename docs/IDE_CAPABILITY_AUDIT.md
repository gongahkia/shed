...
| Docker | Themed workbench for local containers, images, volumes, and networks; it includes inspection, lifecycle, logs, stats, exec/PTY, image pull/run, resource creation, and direct-argv advanced Docker commands. | Docker CLI/daemon and lifecycle remain user-owned; no registry, cloud, Kubernetes, or marketplace surface. |
...
Avoid claims of marketplace compatibility, extension-host support, full remote-development parity, automatic synchronization, or Docker Desktop cloud/Kubernetes/marketplace parity.
