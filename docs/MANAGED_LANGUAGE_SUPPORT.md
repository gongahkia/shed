...
The Language Services panel is available from Settings or `:lsp manage`. An install/update is available only for a catalog entry with a reviewed distribution, and each such action is a separate GUI approval; selecting the panel, opening a file, or running `:lsp manage install <ext>` only opens the local panel and does not start an install.
