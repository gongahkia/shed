...
| Remote workspace connection and synchronization | `:remote open`, `:remote pull`, and `:remote push` are explicit asynchronous actions. | Git, `rsync`, Docker, or a contributed provider owns the protocol. Built-in providers use local mirrors and reject URI passwords. |
| Dev Container CLI | `DevContainerController` runs `:container up` or `:container exec` only after an explicit command in a workspace with a local `devcontainer.json`. | The user-installed `devcontainer` CLI owns Docker/registry/network behavior. Shed sends no workspace data to an app-owned endpoint and does not install the CLI. |
| Notebook execution | `:notebook run` or the notebook's Run all button explicitly starts a local `jupyter nbconvert` process after project trust. | Jupyter owns any kernel/network behavior; Shed sends no notebook content to an app-owned endpoint. |
