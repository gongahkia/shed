...
| `:task cmake [dry-run] <configure\|build\|test> <preset>` | Explicitly run one CMake configure, build, or test preset as direct argv |
...
For a project with `CMakePresets.json` or local `CMakeUserPresets.json` at its workspace root, `:task cmake configure <preset>`, `:task cmake build <preset>`, and `:task cmake test <preset>` run `cmake --preset`, `cmake --build --preset`, or `ctest --preset` directly. Add `dry-run` after `cmake` to inspect the exact direct argv without starting it. CMake, not Shed, resolves preset files, includes, inheritance, conditions, macros, and the local user-preset file. Shed does not list, parse, select, write, or persist presets, infer a preset for ordinary `:task run`, or make a preset available to Test Explorer before it has generated a CTest tree. [CMake Presets](https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html)
