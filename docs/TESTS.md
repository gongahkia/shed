...
| `ctest` | exactly one generated `CTestTestfile.cmake` in the root, `build`, `cmake-build-debug`, `cmake-build-release`, or `out/build` | `ctest --show-only=json-v1` | `ctest --output-junit`; selected tests use an exact escaped `-R` expression |

Auto-detection only inspects local files. It never downloads an executable or dependency. A missing runtime is reported in the selected test job output. Node adapters prefer a local `node_modules/.bin` executable; Maven and Gradle prefer executable wrappers. CTest auto-detection requires one already-generated test tree and CTest 3.21+ for its JUnit report option; Shed neither configures CMake nor builds a target. When there is exactly one conventional generated tree, the normal `:task test` and `:task build` fallbacks run `ctest --test-dir <tree> --output-on-failure` and `cmake --build <tree>` as direct argv. An explicit `:task cmake test <preset>` can instead let CTest resolve a named project or user preset, but Test Explorer does not infer that preset or expose its tests until it finds a generated tree. Multiple recognized generated trees are intentionally ambiguous and require an explicit `.shedtests` command such as `["ctest", "--test-dir", "build"]`.
