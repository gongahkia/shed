...
Remote workspaces and Dev Containers keep their own environment and do not consume these local selections. Shed does not install runtimes, JDKs, compilers, debug adapters, package managers, or language servers; it does not interpret `nvm`, `asdf`, `pyenv`, or similar version-manager configuration.
