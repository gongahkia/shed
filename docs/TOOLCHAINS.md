...
Shed can use one explicitly selected existing Python, Node, and Go executable per local workspace. This is a process-environment selection feature, not a runtime installer, version manager, or remote host.
...
Detection lists conventional Python environments at `.venv`, `venv`, and `env`, plus the first `python`/`python3`, `node`, and `go` executable on Shed's inherited `PATH`. Detection never chooses or runs a candidate. Select an absolute executable only after reviewing it:
