...
Saved `:workspace` profiles also retain ordered `workspaceRoots` and `workspaceActiveRoot` fields in their session payload. Shed renders one selected tree at a time; search, LSP, and Git resolve against the active file or selected root.
