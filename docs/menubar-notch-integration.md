...
| Übersicht/simple-bar | Reserve top bar space; keep the bar display-only. | Use `extensions/ubersicht/olly.widget`; it renders `ollyctl state --json` output once per second. | Widget refresh polling can waste CPU; use `ollyctl events --json` for heavier custom widgets. Custom top-bar height needs explicit reserve. |
