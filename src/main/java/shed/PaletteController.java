...
final class PaletteController {
    ...
    private static final List<PaletteAction> SURFACE_ACTIONS = actions(
        ...
        action("Focus Previous Split", "window previous", "Focus the previous editor split."),
        ...
        action("Grow Current Split", "window grow", "Increase the active split by 5%, or specify a percentage after grow."),
        action("Shrink Current Split", "window shrink", "Decrease the active split by 5%, or specify a percentage after shrink."),
        action("Zoom In", "zoom in default", "Increase the interface scale by the default 10% increment."),
        action("Zoom Out", "zoom out default", "Decrease the interface scale by the default 10% increment."),

        action("Open Quickfix", "quickfix open", "Open the current quickfix list."),
        action("Next Quickfix Result", "quickfix next", "Move to the next quickfix result."),
        action("Previous Quickfix Result", "quickfix previous", "Move to the previous quickfix result."),
        action("First Quickfix Result", "quickfix first", "Move to the first quickfix result."),
        ...
        action("Toggle Minimap", "minimap toggle", "Toggle the minimap panel."),
        action("Undo History", "undo history", "Show the undo history summary."),
        action("Clear Search Highlights", "search highlights clear", "Clear active search highlights."),
        action("Command Log", "command log", "Open the command log buffer."),
        action("Show Help", "help open", "Open the built-in help buffer."),
        action("About Shed", "app about", "Show Shed version and local runtime details."),

        action("Format Current Buffer", "format current", "Format the active buffer with its selected formatter."),
        action("Formatter Policy", "format policy", "Configure the current language's formatter and format-on-save policy."),
        action("Markdown Preview", "markdown preview", "Open the live native Markdown preview beside the source buffer."),
        action("Table of Contents", "document toc", "Open the current Markdown document's table of contents."),
        action("Document Outline", "document outline", "Open the current document outline."),
        action("Toggle Markdown Checkbox", "markdown checkbox toggle", "Toggle the checkbox at the caret."),
        action("Insert Markdown Table", "markdown table insert", "Insert the default Markdown table template."),
        action("Insert Link", "markdown link insert", "Insert a link at the caret."),
        action("Insert Image", "markdown image insert", "Choose and insert a local image reference."),
        action("Edit Snippets", "snippet edit", "Open the user snippets buffer for editing."),
        action("Toggle Bracket Colors", "bracket colors toggle", "Toggle matching bracket colorization."),
        action("Integrated Terminal", "terminal open", "Open the integrated terminal split."),
        action("Word Count", "document wordcount", "Show line, word, and character counts."),
        action("Registers", "register list", "Show register contents."),
        action("Yank Ring", "yank ring", "Open the copied and deleted text history."),
        action("Marks", "mark list", "Show marks for the active buffer."),
        ...
        action("Document Symbols", "document symbols", "Open the document-symbol picker, with a local fallback."),

        action("Workspace Folders", "workspace folders", "Open the workspace-folder manager."),
        action("Workspace Index Status", "workspace index status", "Show the workspace search index status."),
        action("Project Replace", "project replace", "Open the reviewed project-wide replacement panel."),
        action("Tasks", "task open", "Open the workspace Tasks panel."),
        action("Tests", "test open", "Open the Test Explorer."),
